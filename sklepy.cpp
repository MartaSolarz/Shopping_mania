...
protected:
    string nazwa;
    double marza;  // marża w przypadku zwyklego sklepu wynosi 0
...
public:
    // Konstruktory:
    Sklep() : JednostkaHandlowa() { adres = Adres(0,0); nazwa_sieci = ""; czy_w_sieci = false; }
    Sklep(string nazwa, Adres wsp) : JednostkaHandlowa(nazwa) { adres = wsp; nazwa_sieci = ""; czy_w_sieci = false; }
    Sklep(string nazwa, Adres wsp, vector<Towar>& wektor_towary) : JednostkaHandlowa(nazwa) { adres = wsp; nazwa_sieci = ""; czy_w_sieci = false; this->towary = wektor_towary; }
